...
class SeasonalAnimeBloc extends Bloc<SeasonalAnimeEvent, SeasonalAnimeState> {
  ...
  void _onFetchSeasonalAnimeBySeason(FetchSeasonalAnimeBySeason event, Emitter<SeasonalAnimeState> emit) async {
    emit(SeasonalAnimeLoading());
    try {
      final response = await http.get(Uri.parse('https://api.jikan.moe/v4/seasons/${event.year}/${event.season}'));
